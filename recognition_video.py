...
import telepot
bot=telepot.Bot("8588535819:AAH1mh64X6cVAyPBX8kngMHNPCoUgOBl6Vg")
rec=1      
class IdData:
                ...
                if len(face_patches) > 0:
                    ...
                    for bb, landmark, matching_id, dist in zip(
                        ...
                        if matching_id is None:
                            ...
                            # Draw bounding box & label on cropped face
                            cv2.rectangle(face_crop, (0, 0), (face_crop.shape[1]-1, face_crop.shape[0]-1), (0, 255, 0), 2)
                            cv2.putText(face_crop, matching_id, (5, face_crop.shape[0]-10),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)

                            # Save only the cropped face
                            cv2.imwrite("matched_face.jpg", face_crop)

                            # Send to Telegram
                            bot.sendMessage("6091703388", f"The identified person is: {matching_id} at raichur")
                            bot.sendPhoto("6091703388", photo=open("matched_face.jpg", "rb"))
